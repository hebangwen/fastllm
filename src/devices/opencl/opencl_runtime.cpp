...
OpenCLRuntime::OpenCLRuntime() {
  ...
  *device_ = devices[0];
  ...
  context_ = std::make_shared<cl::Context>(
      new cl::Context(*device_, nullptr, nullptr, nullptr));
...
void OpenCLRuntime::BuildProgram(const std::string &programName,
                                 ...
                                 cl::Program *program) {
  auto it = openCLProgrameMap_.find(programName);
  AssertInFastLLM(it != openCLProgrameMap_.end(), "program not found");
  auto &programSource = it->second;
...
const std::vector<std::string> &buildOptions, const std::string &kernelName, cl::Kernel *kernel) {
...
cl::Context& OpenCLRuntime::context() const {
    return *context_;
}

cl::CommandQueue& OpenCLRuntime::command_queue() const {
    return *queue_;
}

cl::Device& OpenCLRuntime::device() const {
    return *device_;
}
